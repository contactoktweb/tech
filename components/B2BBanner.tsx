          ...
          <div className="b2b-image-grid">
            {data?.graphics && data.graphics.length > 0 ? (
              data.graphics.map((g, idx) => (
                <div key={idx} className={`b2b-img b2b-img-${idx + 1} animate-on-scroll reveal-geometric`}>
                  {g.svgCode && (
                    <div dangerouslySetInnerHTML={{ __html: g.svgCode }} style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }} />
                  )}
                </div>
              ))
            ) : (
